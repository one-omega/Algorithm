...
public class DijkstraImprove {
    ...
    public DijkstraImprove(EdgeWeightedGraph G, int s, int d, double[][] vLoc) {
        for (Edge e : G.edges()) {
            if (e.weight() < 0)
    ...
    private void relax(Edge e, int v, double[][] vLoc, int d) {
        ...
        double newDist = distTo[v] + e.weight() + MapRouting.distance(w,d) - MapRouting.distance(v,d);
